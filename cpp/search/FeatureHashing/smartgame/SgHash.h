...
template<int N>
uint64_t SgHash<N>::Code64Bit() const
{
    SG_ASSERT(N<=64);//This function is only valide when N<=64
    return static_cast<uint64_t>(m_code.to_ullong());
}
