...
#define MEMORY_COMPUTE_SAVE_FEATURE 0
#define MEMORY_COMPUTE_BACKPROP_VALUE 1
...
SgUctSearch::SgUctSearch(SgUctThreadStateFactory* threadStateFactory,
    ...
    : m_threadStateFactory(threadStateFactory),
      ...
      m_memory_mode(MEMORY_COMPUTE_BACKPROP_VALUE)
{
...
SgUctValue SgUctSearch::GetBoundMemory(bool useRave, const SgUctNode& node,
                          const SgUctNode& child, SgUctThreadState& state) const
{
    SgUctValue posCount = node.PosCount();
    int virtualLossCount = node.VirtualLossCount();
    if (virtualLossCount > 0)
    ...
    return GetBoundMemory(useRave, true, Log(posCount), child, state);
}

SgUctValue SgUctSearch::GetBoundMemory(bool useRave, bool useBiasTerm,
                          SgUctValue logPosCount,
                          const SgUctNode& child, SgUctThreadState& state) const
{
    SgUctValue value;
    ...
    double decayRate = child.GetDecayRate();
    if (m_memory_mode == MEMORY_COMPUTE_SAVE_FEATURE)
    {
        SG_ASSERT(child.HasFeatureBlob());
        double memValue, memCount;
        state.EvaluateMemory(child.GetFeatureBlob(), memValue, memCount);
        value = (1 - decayRate) * value + decayRate * memValue;
        
    }else if(m_memory_mode == MEMORY_COMPUTE_BACKPROP_VALUE)
    {
        value = (1 - decayRate) * value + decayRate * child.MeanMemory();
    }
...
bool SgUctSearch::PlayInTree(SgUctThreadState& state, bool& isTerminal)
{
 //   SgDebug() << "--------------------------------start play in tree--------------------------------------" << '\n';
    ...
    vector<uint64_t>& nodes_hash = state.m_gameInfo.m_nodes_hash;
    
//    shared_ptr<Blob<float> >& feature_blob = state.m_gameInfo.m_feature_blob;
//    SgUctValue& memory_value = state.m_gameInfo.m_memory_value;
//    SgUctValue& memory_count = state.m_gameInfo.m_memory_count;
    ...
    //SgDebug() << "CNN counter: " << state.GetCnnCounter() << '\n';
    ...
    while (true)
    {
        ...
        if (! current->HasChildren())
        {
            ...
            uint64_t hash;
            state.GenerateAllMovesMemory(0, state.m_moves, provenType, hash,feature_blob);
            
            if(!current->HasFeatureBlob() && feature_blob.get() != NULL)
            {
                const_cast<SgUctNode*>(current)->SetFeatureBlob(feature_blob);
            }
            
            //Add new memory entry. Value in memory is always considered in the player's perspective
            if (feature_blob.get() != NULL)
                state.AddNewMemoryEntry(hash, 0.0, 0.0, feature_blob);
            
            nodes_hash.push_back(hash);
                 ...
                 && NeedToComputeKnowledge(current))
        {
            ...
            state.m_moves.clear();
            SgUctProvenType provenType = SG_NOT_PROVEN;
            //bool truncate = state.GenerateAllMoves(current->KnowledgeCount(),state.m_moves,provenType);
            uint64_t hash;
            bool truncate = state.GenerateAllMovesMemory(current->KnowledgeCount(),state.m_moves,provenType, hash,feature_blob);
        ...
        current = &SelectChildMemory(state.m_randomizeRaveCounter, useBiasTerm, *current, state);
        if (m_virtualLoss && m_numberThreads > 1)
            m_tree.AddVirtualLoss(*current);
        nodes.push_back(current);
        SgMove move = current->Move();
        //state.Execute(move)
        uint64_t hashInTree = state.ExecuteMemory(move);
        if(!breakAfterSelect)
        {
            nodes_hash.push_back(hashInTree);
        }
...
const SgUctNode& SgUctSearch::SelectChildMemory(int& randomizeCounter, bool useBiasTerm, const SgUctNode& node, SgUctThreadState& state)
{
    bool useRave = m_rave;
    if (m_randomizeRaveFrequency > 0 && --randomizeCounter == 0)
    {
    ...
    for (SgUctChildIterator it(m_tree, node); it; ++it)
    {
        ...
        if (! child.IsProvenWin()) // Avoid losing moves
        {
            SgUctValue bound;
            //SgDebug() << "mmmmmmmm" << '\n';
            if (child.HasFeatureBlob())
            {
                bound = GetBoundMemory(useRave, useBiasTerm,logPosCount, child, state)
                                        - predictorWeight * child.PredictorValue();
            }
            else
                bound = GetBound(useRave, useBiasTerm,logPosCount, child)
                        - predictorWeight * child.PredictorValue();
...
void SgUctSearch::UpdateTreeMemory(const SgUctGameInfo& info, SgUctThreadState& state)
{
    SgUctValue eval = 0;
    for (size_t i = 0; i < m_numberPlayouts; ++i)
        eval += info.m_eval[i];
    ...
    SgUctValue memory_value = 0.0;
    SgUctValue memory_count = 0.0;
    SgUctValue inverse_memory_value;
    int memory_eval_done = 0;
    //shared_ptr<Blob<float> > feature_blob = info.m_feature_blob;
    const vector<uint64_t> nodes_hash = info.m_nodes_hash;
    
    SG_ASSERT((nodes.size() == nodes_hash.size()) || (nodes_hash.size()+1 == nodes.size()));
    
    //backpropagte result from simulations
    for (size_t i = 0; i < nodes.size(); ++i)
    {
        const SgUctNode& node = *nodes[i];
        const SgUctNode* father = (i > 0 ? nodes[i - 1] : 0);
    ...
    //Add new memory entry. Value in memory is always considered in the player's perspective
//    if (feature_blob.get() != NULL)
//        state.AddNewMemoryEntry(nodes_hash.back(), eval, count, feature_blob);
//
//    
    //update memory statistics
    for (int i = nodes_hash.size()-1; i >= 0; --i)
    {
        const SgUctNode& node = *nodes[i];
        uint64_t hash = nodes_hash[i];

        //update statistics in the memory
        state.UpdateMemoryEntry(hash, eval, count);
        
        if (!node.HasFeatureBlob())
            continue;
        else
        {
            if( memory_eval_done == 0)
            {
                state.EvaluateMemory(node.GetFeatureBlob(), memory_value, memory_count);
                inverse_memory_value = InverseEval(memory_value);
                memory_eval_done = 1;
                //SgDebug() << "memory_value: " << memory_value << ", memory_count: " << memory_count << '\n';
                //SgDebug() << "memory_eval_done" << memory_eval_done << '\n';
            }
            
            if (m_memory_mode == MEMORY_COMPUTE_BACKPROP_VALUE)
            {
                SG_ASSERT(memory_eval_done);
                const_cast<SgUctNode&>(node).AddGameResultsMemory(i % 2 == 0 ? memory_value : inverse_memory_value, memory_count);
            }
        }
    }
...
//    if (m_memory_mode == MEMORY_COMPUTE_BACKPROP_VALUE)
//    {
//        SgDebug() << "memory value: " << memory_value << ", memory count: " << memory_count << '\n';
//    }
}
