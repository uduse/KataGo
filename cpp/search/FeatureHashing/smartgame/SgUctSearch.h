...
struct SgUctGameInfo
{
    ...
    //The following struct members are used for CNN-Memory
    
    //get the feature blob when reaching a leaf node
//    shared_ptr<Blob<float> > m_feature_blob;
    
    std::vector<uint64_t> m_nodes_hash;//the zobrist hash of nodes in m_nodes
    
//    SgUctValue m_memory_value;
//    
//    SgUctValue m_memory_count;
};
...
public:
    ...
    //for CNN-Memory
    virtual bool GenerateAllMovesMemory(SgUctValue count, std::vector<SgUctMoveInfo>& moves,
                                  SgUctProvenType& provenType, uint64_t& hash,
                                  shared_ptr<Blob<float> >& feature_blob) = 0;
    
    virtual void ResetCnnCounter() = 0;
    
    virtual int GetCnnCounter() = 0;
    
    virtual void UpdateMemoryEntry(const uint64_t hash, const SgUctValue value, const SgUctValue count) = 0;

    virtual void AddNewMemoryEntry(const uint64_t key, const SgUctValue value,
                                   const SgUctValue count, const shared_ptr<Blob<float> > rep) = 0;
    
    virtual void EvaluateMemory(const Blob<float> * feature_blob, SgUctValue& memoryValue, SgUctValue& memoryCount) = 0;

    virtual uint64_t ExecuteMemory(SgMove move) = 0;
    
    //virtual void TestMemory() = 0;
    
    //virtual void TestFeatureBlob(const shared_ptr<Blob<float> > feature_blob) = 0;
...
public:
    ...
    */
    ...
    SgUctValue GetBoundMemory(bool useRave, const SgUctNode& node,
...
private:
    ...
    /** See UpdateTreeMemory() and GetBoundMemory()
     Two compute mode:
     MEMORY_COMPUTE_SAVE_FEATURE: use the saved feature_blob in the node to calculate value from memory
     MEMORY_COMPUTE_BACKPROP_VALUE: use the value saved in the node which is backpropagated during search
     */
    int m_memory_mode;
    ...
    SgUctValue GetBoundMemory(bool useRave, bool useBiasTerm,
                        SgUctValue logPosCount,
                        const SgUctNode& child, SgUctThreadState& state) const;
    ...
    const SgUctNode& SelectChildMemory(int& randomizeCounter, bool useBiasTerm, const SgUctNode& node, SgUctThreadState& state);
    ...
    void UpdateTreeMemory(const SgUctGameInfo& info, SgUctThreadState& state);
};
