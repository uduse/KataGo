...
public:
    ...
    SgUctValue MoveCountMemory() const;
    ...
    SgUctValue MeanMemory() const;

    /** True, if mean value is defined (move count not zero) */
    bool HasMean() const;
    
    bool HasMeanMemory() const;
    
    bool HasFeatureBlob() const;
    ...
    //The following functions are used for CNN-Memory

    void AddGameResultsMemory(SgUctValue eval, SgUctValue count);
    
    void SetFeatureBlob(const shared_ptr<Blob<float> > feature_blob);
    
    Blob<float>* GetFeatureBlob() const;
...
private:
    ...
    //The following members are used for CNN-Memory
    
    double m_decay_rate;
    
    SgUctStatisticsVolatile m_memory_statistics;
    
    Blob<float> * m_memory_feature_blob;
...
inline SgUctNode::SgUctNode(const SgUctMoveInfo& info)
    : m_statistics(info.m_value, info.m_count),
      ...
      m_decay_rate(DEFAULT_DECAY_RATE),
      m_memory_statistics(0,0),
      m_memory_feature_blob(NULL)
{
    // m_firstChild is not initialized, only defined if m_nuChildren > 0
}

inline SgUctNode::~SgUctNode()
{
    if (m_memory_feature_blob != NULL)
        delete m_memory_feature_blob;
}
...
inline void SgUctNode::AddGameResultsMemory(SgUctValue eval, SgUctValue count)
{
    m_memory_statistics.Add(eval, count);
}

inline void SgUctNode::SetFeatureBlob(const shared_ptr<Blob<float> > feature_blob)
{
    SG_ASSERT(feature_blob.get()!=NULL);
    SG_ASSERT(!HasFeatureBlob());
    m_memory_feature_blob = new Blob<float>(feature_blob->shape());
    m_memory_feature_blob->CopyFrom(*feature_blob);
}

inline Blob<float>* SgUctNode::GetFeatureBlob() const
{
    SG_ASSERT(HasFeatureBlob());
    return m_memory_feature_blob;
}

inline double SgUctNode::GetDecayRate() const
{
    return m_decay_rate * pow(0.99, m_posCount);//TODO: have a decay formula with count of the node
}
...
inline bool SgUctNode::HasMeanMemory() const
{
    return m_memory_statistics.IsDefined();
}

inline bool SgUctNode::HasFeatureBlob() const
{
    if (m_memory_feature_blob == NULL)
        return false;
    else
...
inline SgUctValue SgUctNode::MeanMemory() const
{
    return m_memory_statistics.Mean();
}
...
inline SgUctValue SgUctNode::MoveCountMemory() const
{
    return m_memory_statistics.Count();
}
